...
function prepareOpenapiObject(opts) {
  ...
  if (opts.components)
    openapiObject.components = Object.assign({}, opts.components, {
      schemas: Object.assign({}, opts.components.schemas),
    });
...
function normalizeUrl(url, serverUrls, stripBasePath) {
  ...
  serverUrls.forEach(function (serverUrl) {
    const basePath = serverUrl.startsWith('/')
      ? serverUrl
...
function resolveServerUrls(servers) {
  ...
  for (const server of servers) {
    ...
    for (const [nameInBrackets, name] of matches) {
      ...
      if (value === undefined) {
        throw new Error(
          `Server URL ${originalUrl} could not be resolved. Make sure to provide a default value for each URL variable.`
        );
      }
...
function transformDefsToComponents(jsonSchema) {
  if (typeof jsonSchema === 'object' && jsonSchema !== null) {
    ...
    if (jsonSchema.patternProperties) {
      jsonSchema.additionalProperties = Object.values(
    ...
    Object.keys(jsonSchema).forEach(function (key) {
      if (key === 'properties') {
        Object.keys(jsonSchema[key]).forEach(function (prop) {
          jsonSchema[key][prop] = transformDefsToComponents(
            jsonSchema[key][prop]
          );
        });
      } else if (key === '$ref') {
        jsonSchema[key] = jsonSchema[key].replace(
          'definitions',
          'components/schemas'
        );
...
function plainJsonObjectToOpenapi3(
  container,
  jsonSchema,
  externalSchemas,
  securityIgnores = []
) {
  const obj = transformDefsToComponents(
    resolveLocalRef(jsonSchema, externalSchemas)
  );
  let toOpenapiProp;
  switch (container) {
    case 'cookie':
  ...
  return Object.keys(obj)
    ...
    .map((propKey) => {
      ...
      if (jsonSchema.schema) {
        ...
        if (typeof jsonSchema.schema.required !== 'undefined')
          delete jsonSchema.schema.required;
        // it is needed as description in schema is invalid prop - delete only if needed
        if (typeof jsonSchema.schema.description !== 'undefined')
          delete jsonSchema.schema.description;
      }
      return jsonSchema;
    });
}
...
function resolveCommonParams(
  ...
  if (resolved.$ref && resolved.$ref.startsWith(schemasPath)) {
    ...
    resolved = pathParts.reduce(
      (resolved, pathPart) => resolved[pathPart],
      ref.definitions().definitions
    );
  }

  const arr = plainJsonObjectToOpenapi3(
    container,
    resolved,
    { ...sharedSchemas, ...ref.definitions().definitions },
    securityIgnores
  );
...
function resolveResponse(fastifyResponseJson, produces, ref) {
  ...
  statusCodes.forEach((statusCode) => {
    ...
    if (rawJsonSchema.type !== 'null') {
      if (
        resolved.content &&
        resolved.content[Object.keys(resolved.content)[0]].schema
      ) {
...
function prepareOpenapiMethod(schema, ref, openapiObject) {
  ...
  const securityIgnores = [
    ...
    Object.keys(securitySchemeGroup).forEach((securitySchemeLabel) => {
      const { name, in: category } =
        openapiObject.components.securitySchemes[securitySchemeLabel];
      if (!acc[category]) {
        acc[category] = [];
      }
  ...
  if (schema) {
    ...
    if (schema.querystring)
      resolveCommonParams(
        ...
        securityIgnores.query
      );
    if (schema.body) {
      openapiMethod.requestBody = { content: {} };
      resolveBodyParams(
        openapiMethod.requestBody,
        schema.body,
        schema.consumes,
        ref
      );
  ...
  openapiMethod.responses = resolveResponse(
    schema ? schema.response : null,
    schema ? schema.produces : null,
    ref
  );
...
function prepareOpenapiSchemas(schemas, ref) {
  return Object.entries(schemas).reduce((res, [name, schema]) => {
    const _ = { ...schema };
    const resolved = transformDefsToComponents(
      ref.resolve(_, { externalSchemas: [schemas] })
    );
