...
function plainJsonObjectToSwagger2(
  container,
  jsonSchema,
  externalSchemas,
  securityIgnores = []
...
function resolveBodyParams(parameters, schema, ref) {
  ...
  parameters.push({
    ...
    in: 'body',
    description:
      resolved && resolved.description ? resolved.description : undefined,
    schema: resolved,
  });
}
...
function resolveCommonParams(
  ...
  const resolved = ref.resolve(schema);
  const arr = plainJsonObjectToSwagger2(
    container,
    resolved,
    sharedSchemas,
    securityIgnores
...
function resolveResponse(fastifyResponseJson, ref) {
  ...
  statusCodes.forEach((statusCode) => {
    ...
    if (
      statusCode.toUpperCase().includes('XX') &&
      statusCodes.includes(deXXStatusCode)
    ) {
      return;
    }
    ...
    const response = {
      description:
        rawJsonSchema[xResponseDescription] ||
...
function prepareSwaggerMethod(schema, ref, swaggerObject) {
  ...
  const securityIgnores = [
    ...
    Object.keys(securitySchemeGroup).forEach((securitySchemeLabel) => {
      const { name, in: category } =
        swaggerObject.securityDefinitions[securitySchemeLabel];
      if (!acc[category]) {
        acc[category] = [];
      }
  ...
  if (schema) {
    ...
    if (schema.body) {
      const isConsumesAllFormOnly =
        isConsumesFormOnly(schema) || isConsumesFormOnly(swaggerObject);
  ...
  swaggerMethod.responses = resolveResponse(
